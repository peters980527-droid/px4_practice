...
5. 비용 구하는거 관련
   - 드론이 어떻게 미래에 보이는 바람을 미리 대응을할까? 첫번째 스텝 명령만 가져다가 쓰는데 바람이 19스텝부터 등장한다고 하면 어떻게 거기까지 대응을 할까?
   - 예를들어서 바람이 첫번째 스텝까지 올때까지 기다린다고 치자. 그러면 이 명령들을 모델에 넣고 비용을 계산해보면 자세가 늦게 변해서 바람이 왔을 때 위치가 많이 밀릴 수 있음. 그러면 위치오차 비용이 커짐.
   - 다른 방법으로는 지금부터 조금 씩 기울이는걸 후보로 보는거임. 그리면 미리 기울이는 동안에는 제어 명령 비용이 조금 생기지만 나중에 바람이 왔을 때 위치오차가 작아져서 전체 비용이 작아짐.
   - 이런식으로 후보를 비용이 작아지는 쪽으로 명령들을 맞춰가는거임.
   - 그리고 최적화 결과가 바람 코앞에서 크게 대응이라는걸 내놓을 수도 있는데 지금 모델에는 자세 지연이 있어서 어차피 마지막 스텝에서 갑자기 크게 명령을 줘도 자세와 위치가 제때 바뀌지 않음. 이걸 변화율 계산할때 어차피 다 들어가는거라 비용 계산할때 다 참고함.
   - 근데 계산을 여러번하면서 후보를 여러개두고 비교해가면서 결정하는게 acados가 하는건 아님.
   - ① 임시로 가지고 있는 20개 명령으로 미래를 예측한다.

② 예측 결과가 목표에서 얼마나 벗어나는지 본다.

③ 자코비안이
   “각 명령을 조금 바꾸면 미래 예측이 어느 방향으로 얼마나 변하는지”
   알려주는 계산표를 만든다.

④ QP가 그 계산표를 이용해
   “20개 명령을 각각 얼마나 수정해야 전체 비용이 가장 작아질까?”
   를 계산한다.

⑤ 수정된 첫 번째 명령만 드론에 적용한다.
...
################################################################################
1. PX4에서 현재 상태를 받는다.
2. nmpc는 0.02초마다 20스텝을 계산한다.
3. 우선 20개의 초기 추측 명령값들이 만들어진다.
4. 20개의 명령중 일단 첫번째꺼를 현재 상태랑 외란이 있다면 그거까지 포함해서 첫번째 스텝을 계산함. 계산한 미래 상태가 두번째 스텝에서 쓸 상태임. 즉, 0.1초 뒤
5. 각 스텝을 지날때마다 그 구간의 비용식도 세움. 근데 비용을 바로 계산하는건 아님.
6. (x_k - x_ref)^T는 1x11 행렬. 그리고 Q는 11 x 11행렬. 그리고 (x_k - x_ref) 는 11x1 행렬. 즉 다 곱하면 숫자 하나가 나옴. 이게 비용임.
7. QP 가 각 스텝의 비용식에 수정된 상태와 명령을 추가함. 더해지는 수정된 상태와 명령은 아래에서 설명할 자코비안에서 구한 Ak Bk에 의해 계산되어서 더해짐. 그냥 랜덤하게 더해지는 숫자가 아님.
8. 각 스텝 지날때마다 자코비안도 각각 구함. Ak 행렬과 Bk 행렬을 각각 스텝마다 하나씩, 총 각각 20개씩 구하는 것. Ak는 현재 상태를 조금 바꾸면 다음 상태가 얼마나 바뀌는지 , Bk는 현재 명령을 조금 바꾸면 다음 상태가 얼마나 바뀌는지
9. 자코비안은 복잡한 비선형식을 현재 지점 근처에서 직선처럼 근사하는 방법. 그 직선의 기울기가 바로 “명령을 바꿨을 때 상태가 변하는 효과”로 해석
10. 자코비안으로 근사를 하면 곧바로 “진짜 비선형 문제의 완벽한 최저 명령”이 나오는 건 아니고, 현재 임시 명령 주변에서 비용이 가장 작아지는 방향과 수정량이 나옴.
11. 각각의 스텝에 자코비안으로 더해진 임 상태와 명령이 더해진 비용식이 20개가 이제 생김.
12. 끝에서 20개의 식을 전부 더함.
13. 그리고 QP가 이 전체 합을 가장 작게 만드는 20개의 명령을 구함.
14. 그렇게 수정된 20개의 명령 중 가장 첫번째만 PX4로 실제 명령으로 전달이 되는것.
15. 0.02초 뒤에 px4로부터 변화된 현재 상태를 받고 또 20스텝을 하고... 반복
################################################################################
################################################################################
<img width="583" height="56" alt="image" src="https://github.com/user-attachments/assets/8daf149c-9a29-4aec-bd6e-2947f684e5e3" />

<img width="522" height="65" alt="image" src="https://github.com/user-attachments/assets/a7cd7a5d-b7f3-490d-a1cf-682a285a2ebd" />

<img width="940" height="451" alt="image" src="https://github.com/user-attachments/assets/92bc78dd-2af0-44c7-b4a0-84f3ab0f99a6" />

<img width="481" height="100" alt="image" src="https://github.com/user-attachments/assets/4a85822d-6e64-4f1b-bf23-75a1b28240e0" />
