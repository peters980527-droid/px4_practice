...
5. 각 스텝을 지날때마다 그 구간의 비용도 계산함. 위치가 얼마나 벗어났는지, 명령을 얼마나 크게 썼는지 등등 고려해서.
6. 끝에서 20개의 비용을 전부 더함
7. 각 스텝에서 비용을 구할때는 자코비안까지만 구하고 나중에 끝에서 QP를 이용해서 전체 20개의 비용을 낮추도록 20개 명령을 한 번 수정.
8. 그렇게 수정된 20개의 명령중 가장 첫번째만 PX4로 실제 명령으로 전달이 되는것.
9. 0.02초 뒤에 px4로부터 변화된 현재 상태를 받고 또 20스텝을 하고... 반복
################################################################################
################################################################################
