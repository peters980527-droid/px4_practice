...
5. 비용 구하는거 관련
   ...
   - ③ 자코비안이 “각 명령을 조금 바꾸면 미래 예측이 어느 방향으로 얼마나 변하는지” 알려주는 계산표를 만든다.
   - ④ QP가 그 계산표를 이용해 “20개 명령을 각각 얼마나 수정해야 전체 비용이 가장 작아질까?” 를 계산한다.
   - ⑤ 수정된 첫 번째 명령만 드론에 적용한다.


1. PX4에서 현재 상태를 받는다.
2. nmpc는 0.02초마다 20스텝을 계산한다.
3. 우선 20개의 초기 추측 명령값들이 만들어진다. (NMPC 진입 때 처음 한번 20개를 hover로 초기화. 이후 solver가 기존해를 이어서 사용.)
4. 20개의 명령중 일단 첫번째꺼를 현재 상태랑 외란이 있다면 그거까지 포함해서 첫번째 스텝을 계산함. 계산한 미래 상태가 두번째 스텝에서 쓸 상태임. 즉, 0.1초 뒤
5. 각 스텝을 지날때마다 그 구간의 비용식도 세움. <img width="977" height="159" alt="image" src="https://github.com/user-attachments/assets/fb1d08a0-b28b-4e95-9143-2121fa274882" />
6. 중간에 낀 델타 xk가 바로 이전 스텝에서 구한 상태 수정량임. 이건 <img width="399" height="56" alt="image" src="https://github.com/user-attachments/assets/1ee21ccc-da85-405d-a3d0-0210b08faa92" /> 관계식을 사용함.
7. 식에서 Ak 랑 Bk가 바로 자코비안으로 Δuk: QP가 선택할 명령 수정량, 그리고 Δxk+1: 그 명령 수정으로 발생할 상태 수정량의 둘 사이의 관계를 알려주는 자코비안임.
8. 즉, Ak: 상태를 조금 바꿨을 때 다음 상태가 어떻게 바뀌는지, Bk: 명령을 조금 바꿨을 때 다음 상태가 어떻게 바뀌는지. 즉 Ak,Bk는 QP에게 “이 방향으로 명령을 바꾸면 미래 상태가 이렇게 움직인다”를 알려주는거임.
9. Ak Bk가 최저점을 찾는건 아니고 QP가 Ak Bk 정보 관계를 이용해서 최저점을 찾는것. QP자체가 마지막에 최저점을 찾을수 있게 계산되게끔 세팅되어있는거임.
10. QP는 구간별 비용식을 나중에 다 더해서 한번에 20개의 구간별 비용식이 최저점을 갖는 명령값을 각각 대입해서 찾아냄. 즉, 비용식 안에 delta uk가 현재 스텝에서 QP가 찾아서 넣을 명령값이고 delta xk는 이전 스텝에서 구한 상태 수정량이고 그 안에 들어가있는 delta uk는 이전 스텝에서 QP가 넣은 명령 수정값임.
11. 그렇게 비용을 최저로 갖는 20개의 명령이 모이면 그 중 가장 첫번째 명령만 PX4로 실제 명령으로 보내는것.
